...
contract EventTest is Test {
    ...
    function setUp() public {
        ...
        collateralAmount = eventManager.maxCollateral();
    ...
    function testInitialization() public {
        ...
        assertEq(
            uint256(eventContract.status()),
            uint256(Event.EventStatus.Open)
        );
    }
    ...
    function testSubmitOutcome() public {
        ...
        assertEq(
            uint256(eventContract.status()),
            uint256(Event.EventStatus.Resolved)
        );
    ...
    function testContributeToDispute() public {
        ...
        assertEq(
            uint256(eventContract.disputeStatus()),
            uint256(Event.DisputeStatus.Disputed)
        );
    }
    ...
    function testResolveDisputeOutcomeChanged() public {
        ...
        uint256 collateralBalance = eventManager.collateralBalances(
    ...
    function testResolveDisputeOutcomeUpheld() public {
        ...
        // Adjusted Assertion: Check that the creator's collateral is still locked
        ...
        uint256 maxCollateral = eventManager.maxCollateral(); // 1e23
        int256 creatorReputation = eventManager.creatorsReputation(creator); // Should be 1
        uint256 MAX_REPUTATION = eventManager.MAX_REPUTATION(); // 100

        // Compute the collateral discount
        int256 collateralDiscount = int256(maxCollateral) * creatorReputation / int256(MAX_REPUTATION);
        int256 calculatedCollateralAmount = int256(maxCollateral) - collateralDiscount;

        // Ensure the collateral amount is not negative
        uint256 expectedCollateralAmount = calculatedCollateralAmount >= 0 ? uint256(calculatedCollateralAmount) : 0;

        // Enforce minimum collateral amount if necessary
        uint256 minimumCollateral = 1e18; // Assuming this is the minimum
        if (expectedCollateralAmount < minimumCollateral) {
            expectedCollateralAmount = minimumCollateral;
        }

        assertEq(collateralBalance, expectedCollateralAmount);
    }
    ...
    function testCannotExceedMaxOutcomes() public {
        ...
        for (uint256 i = 0; i < 13; i++) {
            manyOutcomes[i] = string(
                abi.encodePacked("Outcome ", Strings.toString(i))
            );
        }
